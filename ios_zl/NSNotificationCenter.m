...
一次完整的观察过程，关键的类有这么几个（介绍顺序按照完成顺序）：

1. 观察者Observer，一般继承自NSObject，通过NSNotificationCenter的addObserver:selector:name:object接口来注册对某一类型通知感兴趣.在注册时候一定要注意，NSNotificationCenter不会对观察者进行引用计数+1的操作，我们在程序中释放观察者的时候，一定要去报道从center中将其注销了。 
...
而在实际使用过程中，会发现该函数的性能是比较低下的。在整个启动过程中，进行了10000次RemoveObserver操作，如果整个NotificationCenter中混杂着多种消息类型，那么恐怕对于性能来说将会是灾难性的。
...
//DZAppDelegate
DZClient* client = [[DZClient alloc] init];
...
在上面介绍了，观察者被动的接受主题改变的经典意义上的观察者模式之后，我们再来看一下广义观察者模式。当然上面所讲的经典观察者模式，也是一种传递数据的方式。广义观察者涵盖了经典观察者模式。
...
观察者需要实现名为NSKeyValueObserving 的category 方法来处理收到的变更通知：
